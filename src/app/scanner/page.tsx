...
const RULES = [
  {
    ...
    icon: "🎮",
    title: "게임주 — 신작 출시 소식",
    sub: "넥슨게임즈·펄어비스·크래프톤·카카오게임즈·위메이드·네오위즈 · 처음 언급되는 신작만",
  },
  {
    id: "bio",
    icon: "🧬",
    title: "바이오주 — FDA 임상 진입 (국내)",
    sub: "국내 상장 바이오·제약사가 미국 FDA에 임상 1/2/3상 신청(IND)·승인 · 회사→종목 자동 매칭",
  },
];
...
export default function ScannerPage() {
  ...
  const [data, setData] = useState<GameResult | BioResult | null>(null);
  ...
  async function open(id: string) {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/scanner?rule=${id}`, { cache: "no-store" });
    ...
    <main className="mx-auto max-w-3xl px-5 py-10">
      <header className="mb-6 flex items-end justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-zinc-400">
            재료를 고르면 <span className="text-indigo-400">오늘 발동</span> 여부와 과거 반응을
            보여줘요.
          </p>
        </div>
        ...
        <div className="space-y-3">
          {RULES.map((r) => (
            <button
              key={r.id}
              onClick={() => open(r.id)}
              className="flex w-full items-center gap-4 rounded-2xl border border-ink-600 bg-ink-800 p-4 text-left transition-colors hover:border-indigo-500"
            >
              <span className="text-2xl">{r.icon}</span>
              <span className="min-w-0 flex-1">
                <span className="block font-semibold text-white">{r.title}</span>
                <span className="mt-0.5 block text-xs text-zinc-500">{r.sub}</span>
              </span>
              <span className="shrink-0 text-sm text-indigo-400">보기 →</span>
            </button>
          ))}
          <p className="pt-2 text-center text-xs text-zinc-600">
            재료를 선택하면 그때 스캔합니다 (약 10~20초)
          </p>
        </div>
      )}
      ...
      {selected && rule && (
        <>
          ...
          {data && selected === "game" && <GameView r={data as GameResult} />}
          {data && selected === "bio" && <BioView r={data as BioResult} />}
        </>
      )}
...
function GameView({ r }: { r: GameResult }) {
  return (
    <div>
      <TodayBox count={r.today.length}>
        {r.today.map((t) => (
...
function BioView({ r }: { r: BioResult }) {
  return (
    <div>
      <TodayBox count={r.today.length}>
        {r.today.map((e, i) => (
